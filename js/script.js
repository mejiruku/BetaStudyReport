...
function showConfirm(message) {
    return new Promise((resolve) => {
        ...
        const handleCancel = () => { cleanup();
            resolve(false); };
        const handleBackdropClick = (e) => { if (e.target === modal) { cleanup();
                resolve(false); } };
...
function showExportConfirm() {
    return new Promise((resolve) => {
        ...
        const handleWithLogs = () => { cleanup();
            resolve("with_logs"); };
        const handleNoLogs = () => { cleanup();
            resolve("no_logs"); };
        const handleCancel = () => { cleanup();
            resolve("cancel"); };
        const handleBackdropClick = (e) => { if (e.target === modal) { cleanup();
...
function addSubject(initialData = null) {
    ...
    <div class="form-group">
        ...
        <select class="subject-select" onchange="toggleOtherInput(this)">
            ${subjectList.map((s) => { const val = s === "選択してください" ? "" : s; return `<option value="${val}">${s}</option>`; }).join("")}
        </select>
  ...
  textarea.addEventListener("input", function () { autoResize(this); generateText(); });
  if (initialData) {
...
function toggleOtherInput(selectElement) {
  ...
  if (selectElement.value === "その他") { otherInput.style.display = "block"; } else { otherInput.style.display = "none"; otherInput.value = ""; }
  generateText();
}
...
function toggleViewMode() { isViewMode = !isViewMode; applyViewMode(isViewMode); saveViewModePreference(isViewMode); }
function applyViewMode(viewMode) {
  ...
  if (viewMode) {
    ...
    const outputText = document.getElementById("output-text");
    if (copyBtn && outputText) outputText.parentNode.insertBefore(copyBtn, outputText.nextSibling);
  } else {
    ...
    const copyBtn = document.querySelector(".copy-btn");
    const outputText = document.getElementById("output-text");
    if (copyBtn && outputText) outputText.parentNode.insertBefore(copyBtn, outputText);
  }
}
function saveViewModePreference(viewMode) {
  if (currentUser) {
    db.collection("users").doc(currentUser.uid).set({ viewMode: viewMode }, { merge: true }).then(() => console.log("View mode saved to cloud")).catch((err) => console.error("Failed to save view mode", err));
  } else { localStorage.setItem("studyReportViewMode", viewMode ? "true" : "false"); }
}
...
function generateText() {
  ...
  if (saveTimer) clearTimeout(saveTimer);
  saveTimer = setTimeout(() => { performSave(currentDateStr, saveDataArray, globalComment); }, 1500);
}
...
function performSave(dateKey, subjects, comment) {
  ...
  if (currentUser) { saveToFirestoreWithLog(dateKey, subjects, comment, changeDetail); } else { saveToLocalStorageWithLog(dateKey, subjects, comment, changeDetail); }
}

function detectChanges(dateKey, newSubjects, newComment) {
  let oldData = null;
  if (currentUser) { oldData = window._lastLoadedData || null; } else { const allData = getAllData(); oldData = allData[dateKey] || null; }
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (!oldSub && newSub && newSub.select) { const subjectName = newSub.select === "その他" ? newSub.other || "その他" : newSub.select; changes.push(`${subjectName}を追加`); }
    else if (oldSub && !newSub) { const subjectName = oldSub.select === "その他" ? oldSub.other || "その他" : oldSub.select; changes.push(`${subjectName}を削除`); }
      ...
      if (oldSub.select !== newSub.select || oldSub.other !== newSub.other) { changes.push(`教科を「${oldName}」→「${newName}」に変更`); }
      else if (oldSub.text !== newSub.text) { changes.push(`${newName}: 内容を編集`); }
      else if (oldSub.h !== newSub.h || oldSub.m !== newSub.m) { changes.push(`${newName}: 時間を変更`); }
    }
  }
  if (oldComment !== newComment) { if (!oldComment && newComment) changes.push("コメントを追加"); else if (oldComment && !newComment) changes.push("コメントを削除"); else changes.push("コメントを編集"); }
  return changes.length > 0 ? changes.join(", ") : "軽微な変更";
}

function saveToFirestoreWithLog(dateKey, subjects, comment, changeDetail) {
  if (!currentUser) { isSaving = false; return; }
  const docRef = db.collection("users").doc(currentUser.uid).collection("reports").doc(dateKey);
  docRef.set({ subjects: subjects, comment: comment, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }).then(() => {
    console.log("Saved to Firestore");
    isSaving = false;
    updateSaveStatus("saved");
    addSyncLog("edit", dateKey, changeDetail);
    window._lastLoadedData = { subjects, comment };
  }).catch((err) => { console.error("Error saving", err); isSaving = false; updateSaveStatus("error"); });
}

function getAllData() {
  const json = localStorage.getItem("studyReportAllData");
  if (!json) return {};
  try { return JSON.parse(json); } catch (e) { console.error("Data parse error", e); return {}; }
}
...
function saveToLocalStorageWithLog(dateKey, subjects, comment, changeDetail) {
  ...
  } catch (e) { console.error(e); isSaving = false; updateSaveStatus("error"); }
}
...
function loadData() {
  ...
  if (currentUser) {
    ...
    db.collection("users").doc(currentUser.uid).collection("reports").doc(dateKey).get().then((doc) => {
      ...
      if (doc.exists) { renderData(doc.data()); } else { renderData(null); }
    }).catch((err) => { console.error("Error loading", err); if (dateInput.value === requestedDateKey) renderData(null); });
  } else {
...
function renderData(dayData) {
  isLoading = true;
  container.innerHTML = "";
  if (dayData) { window._lastLoadedData = { subjects: dayData.subjects || [], comment: dayData.comment || "" }; } else { window._lastLoadedData = null; }
  if (dayData) {
    globalCommentInput.value = dayData.comment || "";
    if (dayData.subjects && dayData.subjects.length > 0) { dayData.subjects.forEach((sub) => addSubject(sub)); } else { addSubject(); }
  } else { globalCommentInput.value = ""; addSubject(); }
  generateText();
  document.querySelectorAll("textarea").forEach((textarea) => { autoResize(textarea); setTimeout(() => autoResize(textarea), 0); });
  isLoading = false;
  updateSaveStatus("saved");
}
...
async function resetData() {
  ...
  if (confirmed) {
    ...
    const allData = getAllData();
    if (currentUser) {
      db.collection("users").doc(currentUser.uid).collection("reports").doc(dateKey).delete().then(() => { resetUI(); }).catch((err) => console.error("Error deleting", err));
    } else { try { delete allData[dateKey]; localStorage.setItem("studyReportAllData", JSON.stringify(allData)); resetUI(); } catch (e) { console.error(e); } }
  }
}
function resetUI() {
  isLoading = true;
...
function copyToClipboard() {
  ...
  navigator.clipboard.writeText(copyTarget.value).then(() => {
    ...
    if (isSpecialCodeEnabled && specialCode && specialCode.trim() !== "") {
      ...
      if (newWindow) { newWindow.document.write(specialCode); newWindow.document.close(); showPopup("コピーしました"); } else { showPopup("ポップアップがブロックされました。設定を確認してください。"); }
    } else { showPopup("コピーしました"); }
  }).catch((err) => { console.error("Failed to copy text: ", err); showPopup("コピーに失敗しました"); });
}

function getSpecialCodeEnabled() { if (currentUser) { return window._cachedSpecialCodeEnabled !== false; } else { return localStorage.getItem("studyReportSpecialCodeEnabled") === "true"; } }
function getSpecialCode() { if (currentUser) { return window._cachedSpecialCode || ""; } else { return localStorage.getItem("studyReportSpecialCode") || ""; } }
function openSettings() {
  ...
  if (versionDisplay) { const metaVersion = document.querySelector('meta[name="data-app-version"]'); const version = metaVersion ? metaVersion.getAttribute("content") : ""; if (version) versionDisplay.textContent = `Ver. ${version}`; }
...
function toggleSpecialCodeInput() { const toggle = document.getElementById("special-code-toggle"); const codeInput = document.getElementById("special-code-input"); if (toggle.checked) { codeInput.disabled = false; codeInput.style.opacity = "1"; } else { codeInput.disabled = true; codeInput.style.opacity = "0.5"; } }
function closeSettings() { const modal = document.getElementById("settings-modal"); modal.classList.remove("show"); }
async function saveSettings() {
  ...
  if (currentUser) {
    try {
      await db.collection("users").doc(currentUser.uid).set({ specialCode: specialCode, specialCodeEnabled: isEnabled }, { merge: true });
      window._cachedSpecialCode = specialCode;
      window._cachedSpecialCodeEnabled = isEnabled;
      console.log("Settings saved to cloud");
    } catch (err) { console.error("Failed to save settings", err); showPopup("設定の保存に失敗しました"); return; }
  } else { localStorage.setItem("studyReportSpecialCode", specialCode); localStorage.setItem("studyReportSpecialCodeEnabled", isEnabled); }
  closeSettings();
  showPopup("設定を保存しました");
}
async function loadSettings() {
  if (currentUser) {
    try {
      const doc = await db.collection("users").doc(currentUser.uid).get();
      if (doc.exists) { window._cachedSpecialCode = doc.data().specialCode || ""; window._cachedSpecialCodeEnabled = doc.data().specialCodeEnabled === true; } else { window._cachedSpecialCode = ""; window._cachedSpecialCodeEnabled = false; }
    } catch (err) { console.error("Failed to load settings", err); window._cachedSpecialCode = ""; }
  }
}
  ...
  try {
    ...
    if (currentUser) {
      ...
      if (includeLogs) {
        ...
        logsData = logsSnapshot.docs.map((doc) => { const d = doc.data(); return { ...d, createdAt: d.createdAt ? d.createdAt.toMillis ? d.createdAt.toMillis() : d.createdAt : null }; });
      }
    } else {
      const localReports = localStorage.getItem("studyReportAllData");
      if (localReports) { reportsData = JSON.parse(localReports); }
      if (includeLogs) { logsData = getSyncLogs(); }
    }
    const exportObj = { version: "1.0", exportedAt: new Date().toISOString(), data: { reports: reportsData, logs: logsData } };
    downloadJSON(exportObj, `study_report_backup_${new Date().toISOString().split("T")[0]}.rep`);
    updateSaveStatus("saved");
  } catch (err) { console.error("Export failed", err); showPopup("データ書き出しに失敗しました。"); updateSaveStatus("error"); }
}
...
function importData(input) {
  ...
  reader.onload = async function (e) {
    try {
      ...
      if (!parsed.data || !parsed.data.reports || !Array.isArray(parsed.data.logs)) { throw new Error("Invalid format"); }
      const confirmed = await showConfirm("現在のデータを上書きして取り込みますか？\n(.rep のファイルのみ対応しています)");
      if (confirmed) {
        if (currentUser) { await importToCloud(parsed.data); } else {
    ...
    } catch (err) { console.error(err); if (err.message === "Invalid format") { showPopup("無効なファイル形式です。\n新しい .rep 形式のファイルのみ読み込めます。"); } else { showPopup("ファイルの読み込みに失敗しました。"); } }
    input.value = "";
  };
...
async function importToCloud(dataContainer) {
  ...
  try {
    ...
    for (const dateKey of Object.keys(reports)) {
      ...
      if (!docData.updatedAt) { docData.updatedAt = firebase.firestore.FieldValue.serverTimestamp(); }
    ...
    for (const log of logs) {
      const newLog = { ...log };
      if (newLog.createdAt && typeof newLog.createdAt === "number") { try { newLog.createdAt = new Date(newLog.createdAt); if (isNaN(newLog.createdAt.getTime())) throw new Error("Invalid Date"); } catch (e) { newLog.createdAt = new Date(); } } else { newLog.createdAt = new Date(); }
  ...
  } catch (err) { console.error("Cloud import failed", err); showPopup("一部のデータの取り込みに失敗しました。"); updateSaveStatus("error"); }
}

async function syncDataOnLogin() {
  updateSaveStatus("saving");
  const localData = getAllData();
  if (Object.keys(localData).length === 0) { loadData(); return; }
  try {
    const cloudData = await fetchAllCloudData();
    const { toUpload, toDownload } = compareAndMerge(localData, cloudData);
    if (Object.keys(toUpload).length > 0) { await uploadToCloud(toUpload); }
    localStorage.removeItem("studyReportAllData");
    if (Object.keys(toUpload).length > 0 || Object.keys(toDownload).length > 0) { addSyncLog("sync", "", `同期完了: ${Object.keys(toUpload).length}件アップロード, ${Object.keys(toDownload).length}件はクラウドを優先`); }
    loadData();
  } catch (err) { console.error("Sync failed", err); updateSaveStatus("error"); showPopup("同期に失敗しました。クラウドからデータを読み込みます。"); localStorage.removeItem("studyReportAllData"); loadData(); }
}
async function fetchAllCloudData() {
  if (!currentUser) return {};
  const snapshot = await db.collection("users").doc(currentUser.uid).collection("reports").get();
  const cloudData = {};
  snapshot.forEach((doc) => { cloudData[doc.id] = doc.data(); });
  return cloudData;
}
function compareAndMerge(localData, cloudData) {
  ...
  allDates.forEach((dateKey) => {
    ...
    if (local && !cloud) { toUpload[dateKey] = local; addSyncLog("upload", dateKey, "ローカルからクラウドへアップロード"); }
    else if (!local && cloud) { toDownload[dateKey] = cloud; }
    else if (local && cloud) {
      const localTime = local.updatedAt || 0;
      let cloudTime = 0;
      if (cloud.updatedAt) { if (cloud.updatedAt.toMillis) { cloudTime = cloud.updatedAt.toMillis(); } else if (typeof cloud.updatedAt === "number") { cloudTime = cloud.updatedAt; } }
      if (localTime > cloudTime) { toUpload[dateKey] = local; addSyncLog("upload", dateKey, "ローカルが新しいためアップロード"); } else { toDownload[dateKey] = cloud; }
    }
  });
...
async function uploadToCloud(dataToUpload) {
  ...
  const reportsRef = db.collection("users").doc(currentUser.uid).collection("reports");
  const promises = [];
  Object.keys(dataToUpload).forEach((dateKey) => { const data = dataToUpload[dateKey]; promises.push(reportsRef.doc(dateKey).set({ subjects: data.subjects, comment: data.comment, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })); });
  await Promise.all(promises);
}

function getSyncLogs() { const logsJson = localStorage.getItem("studyReportSyncLogs"); if (!logsJson) return []; try { return JSON.parse(logsJson); } catch (e) { return []; } }
function saveSyncLogs(logs) { localStorage.setItem("studyReportSyncLogs", JSON.stringify(logs)); }
function addSyncLog(action, dateKey, detail) {
  const now = new Date();
  const timestamp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")} ${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
  const logData = { timestamp: timestamp, action: action, date: dateKey || "", detail: detail || "", createdAt: firebase.firestore.FieldValue.serverTimestamp() };
  if (currentUser) {
    db.collection("users").doc(currentUser.uid).collection("logs").add(logData).then(() => console.log("Log added to cloud")).catch((err) => console.error("Failed to add cloud log", err));
  } else { const logs = getSyncLogs(); delete logData.createdAt; logs.unshift(logData); saveSyncLogs(logs); }
}
...
async function showSyncLog() {
  ...
  if (currentUser) {
    try {
      const snapshot = await db.collection("users").doc(currentUser.uid).collection("logs").orderBy("createdAt", "desc").limit(50).get();
      logs = snapshot.docs.map((doc) => { const data = doc.data(); return { timestamp: data.timestamp, action: data.action, date: data.date, detail: data.detail }; });
    } catch (err) { console.error("Failed to fetch logs", err); logList.innerHTML = '<div class="sync-log-empty">ログの取得に失敗しました</div>'; return; }
  } else { logs = getSyncLogs(); }
  if (logs.length === 0) { logList.innerHTML = '<div class="sync-log-empty">操作ログはありません</div>'; } else {
    logList.innerHTML = logs.map((log) => {
      const actionLabel = log.action === "upload" ? "アップロード" : log.action === "download" ? "ダウンロード" : log.action === "edit" ? "編集" : log.action === "sync" ? "同期" : log.action;
      return `
...
function closeSyncLogModal() { const modal = document.getElementById("sync-log-modal"); modal.classList.remove("show"); }
  ...
  if (confirmed) {
    if (currentUser) {
      try {
        ...
        showSyncLog();
      } catch (err) { console.error("Failed to delete logs", err); showPopup("ログの削除に失敗しました"); }
    } else { localStorage.removeItem("studyReportSyncLogs"); showSyncLog(); showPopup("操作ログを削除しました"); }
  }
}